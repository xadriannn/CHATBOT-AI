...
export async function updateProfile(req, res) {
  try {
    const currentUsername = req.user.username; // dari JWT
    const { username, email, password } = req.body;

    if (!username || !email) {
      return res.status(400).json({ success: false, error: "Username dan email wajib diisi." });
    }
    ...
    db.run(query, params, function (err) {
      ...
      res.json({
        ...
        token: newToken // kirim ke frontend
      });
    });
  } catch (error) {
