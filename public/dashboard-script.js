document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateUserStatsAndChart(users) {
    ...
    const newUsers = users.filter((u) => {
      ...
      const userDate = new Date(new Date(u.created_at).toLocaleString("en-US", { timeZone: "Asia/Bangkok" }));
    ...
    users.forEach((u) => {
      if (u.created_at) {
        const d = new Date(new Date(u.created_at).toLocaleString("en-US", { timeZone: "Asia/Bangkok" }));
        const key = d.getFullYear() + "-" + String(d.getMonth() + 1).padStart(2, "0") + "-" + String(d.getDate()).padStart(2, "0");
  ...
  async function fetchUsers() {
    try {
      ...
      users.forEach((user) => {
        ...
        let createdAtStr = "-";
        if (user.created_at) {
          const d = new Date(new Date(user.created_at).toLocaleString("en-US", { timeZone: "Asia/Bangkok" }));
          createdAtStr = d.getFullYear() + "-" + String(d.getMonth() + 1).padStart(2, "0") + "-" + String(d.getDate()).padStart(2, "0") + " " + d.getHours().toString().padStart(2, "0") + ":" + d.getMinutes().toString().padStart(2, "0");
        }
        tr.innerHTML = `
        ...
        tr.innerHTML = `
          ...
          <td class="action-cell">
            <div class="file-action-buttons">
              <a href="/uploads/${encodeURIComponent(file.name)}" download class="primary-btn">Download</a>
  ...
  // ===================================================================
  // FUNGSI AKSI DENGAN INTEGRASI POP-UP KUSTOM
  // ===================================================================
  window.logout = async function () {
    const isConfirmed = await PopupNotify.confirm("Anda akan keluar dari sesi admin dan diarahkan ke halaman login.", "Anda yakin ingin logout?");
    if (isConfirmed) {
  ...
  window.deleteAdmin = async function (username) {
    const isConfirmed = await PopupNotify.confirm(`Tindakan ini tidak dapat diurungkan.`, `Hapus admin ${username}?`);
    if (isConfirmed) {
      try {
  ...
  window.deleteUser = async function (username) {
    const isConfirmed = await PopupNotify.confirm(`Tindakan ini akan menghapus user ${username} secara permanen.`, "Yakin ingin menghapus?");
    if (isConfirmed) {
      try {
  ...
  window.deleteFileFromManager = async function (filename) {
    const isConfirmed = await PopupNotify.confirm(`File "${filename}" akan dihapus secara permanen.`, "Yakin ingin menghapus file?");
    if (isConfirmed) {
      try {
  ...
  document.getElementById("delete-user-button").addEventListener("click", async () => {
    const checked = Array.from(document.querySelectorAll(".select-user:checked"));
    if (checked.length === 0) {
      PopupNotify.info("Pilih minimal satu user yang ingin dihapus.", "Tidak ada user terpilih");
      return;
    }
    const isConfirmed = await PopupNotify.confirm(`Anda akan menghapus ${checked.length} user secara permanen.`, "Hapus user terpilih?");
    if (isConfirmed) {
      let successCount = 0;
      let failedCount = 0;
      ...
      if (failedCount > 0) {
        PopupNotify.failed(`${failedCount} user gagal dihapus.`, "Sebagian Gagal");
      } else {
  ...
  document.getElementById("upload-form").addEventListener("submit", async function (e) {
    ...
    try {
      ...
      if (res.ok) {
        PopupNotify.success(`${fileInput.files.length} file berhasil diupload.`);
        fileInput.value = "";
